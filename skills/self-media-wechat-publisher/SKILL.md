...
3. 记录最终人工审批和单篇草稿授权。不得使用批量或无人值守发布模式。
4. `wenyan publish -f 文章.md -t 主题 -h 高亮主题` 写入单篇草稿箱。
5. 在公众号后台和移动端预览中核对标题、封面、前两段、图片数量、外链、字号、留白、表格和组件是否溢出。
6. 把草稿创建时间、使用主题、移动端预览和人工核对结果写回任务卡。

发布失败时按 [wenyan-setup.md](references/wenyan-setup.md) 排查。同一错误不盲目重试超过一次，凭据类问题交给用户处理。

## 多账号

发布到多个公众号时，改用 wenyan 的凭据配置管理多账号，publish 时加 `--app-id` 指定。每个目标账号都需要单独写入授权和独立 IP 白名单，不能把一个账号的授权扩展到其他账号。

## 安全边界

- 只写草稿箱，不调用群发接口。
- 不形成无人值守的生成、排版、连续发布闭环；每篇都需要人工终审和独立授权。
